...
从嵌套来看，一共是有 3 层的判断，我们应该遵循**当发现无效条件时尽早返回**
...
我觉得：我会选择前一个版本（条件 2 有嵌套）。原因在于：这样的代码比较简短和直白，一个嵌套的 if 使得结构更加清晰。条件反转会导致更多的思考过程（增加认知负担）。 ##三、使用函数默认参数和解构
