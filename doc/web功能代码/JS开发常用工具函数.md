...
export default {
  ...
  isIE9: this.UA && this.UA.indexOf('msie 9.0') > 0,
  isEdge: this.UA && this.UA.indexOf('edge/') > 0,
  isAndroid: (this.UA && this.UA.indexOf('android') > 0) || (weexPlatform === 'android'),
  isIOS: (this.UA && /iphone|ipad|ipod|ios/.test(this.UA)) || (weexPlatform === 'ios'),
  isChrome: this.UA && /chrome\/\d+/.test(this.UA) && !this.isEdge,
  /**识别各种浏览器及平台 */
  ...
  // 判断数据是不是数组类型的数据
  // 将isArray挂载到Array上 Array.isArray = Array.isArray || isArray;
  isArray(arr) {
  ...
  // 判断 value 是不是浏览器内置函数
  // 内置函数toString后的主体代码块为 [native code]，而非内置函数则为相关代码
  // 所以非内置函数可以进行拷贝(toString后掐头去尾再由Function转)
  isNative(value) {
  ...
  // 横线转驼峰命名
  // ab-cd-ef ==> abCdEf
  // 使用记忆函数 let _camelize = cached(camelize)
  camelizeRE: /-(\w)/g,
  camelize(str) {
    return str.replace(this.camelizeRE, (_, c) => {
      return c ? c.toUpperCase() : ""
    })
  },
  // 驼峰命名转横线命名：拆分字符串，使用 - 相连，并且转换为小写
  // abCd ==> ab-cd
  // 使用记忆函数 let _hyphenate = cached(hyphenate);
  hyphenateRE: /\B([A-Z])/g,
  hyphenate(str) {
  ...
  // 字符串首位大写
  // abc ==> Abc
  // 使用记忆函数 let _capitalize = cached(capitalize)
  capitalize(str) {
  ...
  // 简单的深克隆 let clone = JSON.parse( JSON.stringify(target) )
  assign() {
  ...
  // 克隆数据，可深度克隆
  // 列出了原始类型，时间、正则、错误、数组、对象的克隆规则
  clone(value, deep) {
    if (this.isPrimitive(value)) {
      return value
    }
    if (this.isArrayLike(value)) { // 类数组
...
// 生成一个重复的字符串，有 n 个 str 组成，可修改为填充为数组等
repeat(str, n) {
let res = ""
while (n) {
if (n % 2 === 1) {
...
// 将指定字符串由一种时间格式转化为另一种
// dateStrForma('20190626', 'YYYYMMDD', 'YYYY 年 MM 月 DD 日') ==> 2019 年 06 月 26 日
// dateStrForma('121220190626', '----YYYYMMDD', 'YYYY 年 MM 月 DD 日') ==> 2019 年 06 月 26 日
// dateStrForma('2019 年 06 月 26 日', 'YYYY 年 MM 月 DD 日', 'YYYYMMDD') ==> 20190626
dateStrForma(str, from, to) {
str += ''
let Y = ''
if (~(Y = from.indexOf('YYYY'))) {
...
// 检查数据是否是非数字值
// 原生的 isNaN 会把参数转换成数字(valueof)
// 而 null、true、false 以及长度小于等于 1 的数组(元素为非 NaN 数据)会被转换成数字
// Symbol 类型的数据不具有 valueof 接口，所以 isNaN 会抛出错误，这里放在后面，可避免错误
\_isNaN(v) {
return !(typeof v === 'string' || typeof v === 'number') || isNaN(v)
},
// 求取数组中非 NaN 数据中的最大值
...
/\*
使用 Object.assign 可以浅克隆一个对象：let clone = Object.assign({}, target)
...
- \*/
  ...
  function Set(arr) {
  this.items = arr ? unique(arr) : [];
  this.size = this.items.length; // Array 的大小
  }
  Set.prototype = {
  add: function (value) {
  // 添加元素,若元素已存在,则跳过，返回 Set 结构本身。
...
// 一般的也可以使用正则来实现
//'2019 年 06 月 26 日'.replace(/(\d{4})年(\d{2})月(\d{2})日/, '$1-$2-$3') ==> 2019-06-26
...
- \*/
  ...
  if (strict) {//开启严格模式，没找到对应 key 值，抛出错误
  throw new Error('please transfer a valid prop path to form item!');
  }
  break;
  }
  ...
  o: tempObj, //原始数据
  k: keyArr[i], //key 值
  v: tempObj ? tempObj[keyArr[i]] : null // key 值对应的值
  };
  };
...
window.oRequestAnimationFrame ||
function (callback) {
//为了使 setTimteout 的尽可能的接近每秒 60 帧的效果
window.setTimeout(callback, 1000 / 60);
};
...
/\*
36、\_isNaN：检查数据是否是非数字值
原生的 isNaN 会把参数转换成数字(valueof)，而 null、true、false 以及长度小于等于 1 的数组(元素为非 NaN 数据)会被转换成数字，这不是我想要的
Symbol 类型的数据不具有 valueof 接口，所以 isNaN 会抛出错误，这里放在后面，可避免错误
...
- _/
  ...
  lower = +lower || 0
  upper = +upper || 0
  return Math.random()_ (upper - lower) + lower;
  }
  //random(0, 0.5) ==> 0.3567039135734613
  //random(2, 1) ===> 1.6718418553475423
  //random(-2, -1) ==> -1.4474325452361945
...
event.shiftKey && 121 == event.keyCode || //shift + F10
event.altKey && 115 == event.keyCode || //alt + F4
"A" == event.srcElement.tagName && event.shiftKey //shift + 点击 a 标签
) || (event.returnValue = false)
});
//48、禁止右键、选择、复制
['contextmenu', 'selectstart', 'copy'].forEach(function(ev){
