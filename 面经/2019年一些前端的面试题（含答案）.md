...
Flex即：Flexible Box，弹性布局，用来为盒状模型提供最大的灵活性。可以实现类似**垂直居中**布局。
```
.box{
  display: flex;
}
...
vue数据双向绑定是通过**数据劫持**`Object.defineProperty( )`结合**发布者-订阅者模式**的方式来实现的
...
```
原文链接： [vue的双向绑定原理及实现](https://www.cnblogs.com/libin-1/p/6893712.html)
实现代码地址：
